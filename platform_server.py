...
def builder_html() -> HTMLResponse:
    ...
    html = html.replace("builder.js?v=1", "builder.js?v=5")
    html = inject_before(html, "</head>", '<link rel="stylesheet" href="modern-ui.css?v=2">')
    html = inject_before(html, "</body>", '<script src="map-question-override.js?v=2"></script>')
    html = inject_before(html, "</body>", '<script src="builder-experience.js?v=2"></script>')
    return HTMLResponse(html, headers={"Cache-Control": "no-store"})


def survey_html() -> HTMLResponse:
    html = (BASE_DIR / "survey.html").read_text(encoding="utf-8")
    html = html.replace("survey.js?v=1", "survey.js?v=3")
    html = inject_before(html, "</head>", '<link rel="stylesheet" href="modern-ui.css?v=2">')
    html = inject_before(html, "</body>", '<script src="survey-experience.js?v=2"></script>')
    return HTMLResponse(html, headers={"Cache-Control": "no-store"})
...
def builder_javascript() -> str:
    """Serve the builder with one unified configurable point-map question."""
...
def geocode_place(
    ...
    try:
        ...
        # Fallback for preview reliability. This endpoint is only called on an
        # explicit respondent search action, not for background/autocomplete use.
        try:
